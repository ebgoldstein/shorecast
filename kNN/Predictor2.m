...
for i=1:1462
    ...
    for j=1:length(vectorI)
        %shoreline at t+1
        values(j)=shoredatanew(vectorI(j),1)-shoredatanew(vectorI(j),2); 
    end
    ...
    ToPredict(i+1,1)=ThePrediction+ToPredict(i,1);
    for k=1:PSL
        ToPredict(i+1,k+1)=ToPredict(i,k);
    end
end

plot(ToPredict(1:1462,1),'r')
hold on

EBG2=[ToPredict(1:1462,5) ToPredict(1:1462,6) ToPredict(1:1462,7) ToPredict(1:1462,1) ToPredict(1:1462,4)];
save EBG2 EBG2
